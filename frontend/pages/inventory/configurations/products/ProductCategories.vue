      ...
      <div class="bg-white p-8 rounded-lg shadow-md mt-4 mx-auto max-w-4xl">
        <nav aria-label="breadcrumb" class="w-max">
          <ol class="flex flex-wrap items-center w-full bg-opacity-60 rounded-md bg-transparent p-0 transition-all">
            ...
            <li class="flex items-center text-blue-900 antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-blue-500">
              <nuxt-link to="/inventory/configurations/ProductCategories">
                <p class="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-normal">
                  Product Categories
                </p>
              </nuxt-link>
              ...
              <tbody class="bg-white">
                <tr v-for="category in productCategories" :key="category.id">
                  ...
                  <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                    ...
                    <button @click="deleteProductCategory(category.id)"
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 class="text-lg leading-6 font-medium text-gray-900">Update Product Category</h3>
                  <div class="mt-2">
  ...
  export default {
    ...
    methods: {
      async fetchProductCategories() {
        try {
          const response = await axios.get('http://127.0.0.1:8000/inventorybackend/products/getpc/', {
      ...
      async submitForm() {
        try {
          const response = await axios.post('http://127.0.0.1:8000/inventorybackend/products/createpc/', this.formData, {
            withCredentials: true
          });
      ...
      async updateForm() {
        try {
          const response = await axios.put(`http://127.0.0.1:8000/inventorybackend/products/updatepc/${this.selectedCategory.id}/`, this.selectedCategory, {
            withCredentials: true
          });
      ...
      async deleteProductCategory(id) {
        try {
          await axios.delete(`http://127.0.0.1:8000/inventorybackend/products/deletepc/${id}/`, {
            withCredentials: true
          });
          this.fetchProductCategories();
        } catch (error) {

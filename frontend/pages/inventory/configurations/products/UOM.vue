<template>
      ...
      <div class="bg-white p-8 rounded-lg shadow-md mt-4 mx-auto max-w-4xl">
        <nav aria-label="breadcrumb" class="w-max">
          <ol class="flex flex-wrap items-center w-full bg-opacity-60 rounded-md bg-transparent p-0 transition-all">
            ...
            <li class="flex items-center text-blue-900 antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-blue-500">
              <nuxt-link to="/inventory/configurations/products/UOM">
                <p class="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-normal">
                  Unit Of Measure
                </p>
              </nuxt-link>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full">
              ...
              <tbody class="bg-white">
                <tr v-for="category in unitOfMeasureCategories" :key="category.id">
                  <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                    <div class="flex items-center">
                      <div>
                        <div class="text-sm leading-5 text-blue-900">{{ category.id }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                  ...
                  <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                    <button @click="viewDetails(category)" class="px-5 py-2 mr-2 border-blue-500 border text-blue-500 rounded transition duration-300 hover:bg-blue-700 hover:text-white focus:outline-none">Update</button>
                    <button @click="deleteUnitOfMeasureCategory(category.id)" class="px-5 py-2 bg-red-500 border text-white rounded transition duration-300 hover:bg-red-700 hover:text-white focus:outline-none">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 class="text-lg leading-6 font-medium text-gray-900">Update Unit of Measure Category</h3>
                  <div class="mt-2">
                    <form @submit.prevent="updateForm">
                      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                          <label class="block text-sm font-medium mb-1" for="update-name">Name</label>
                          <input v-model="selectedCategory.name" type="text" id="update-name" name="name" class="block w-full text-sm border border-input rounded-lg p-2 bg-background text-foreground" placeholder="Name" />
                        </div>
                        <div>
                          <label class="block text-sm font-medium mb-1" for="update-description">Description</label>
                          <textarea v-model="selectedCategory.description" id="update-description" name="description" class="block w-full text-sm border border-input rounded-lg p-2 bg-background text-foreground" placeholder="Description"></textarea>
                        </div>
                      </div>
                      <div class="mt-4">
  ...
  export default {
    ...
    methods: {
      async fetchUnitOfMeasureCategories() {
        try {
          const response = await axios.get('http://127.0.0.1:8000/inventorybackend/UoM/get/', {
            withCredentials: true
          });
          this.unitOfMeasureCategories = response.data;
        } catch (error) {
          console.error('Error fetching unit of measure categories:', error.response.data);
        }
      },
      async submitForm() {
        try {
          const response = await axios.post('http://127.0.0.1:8000/inventorybackend/UoM/create/', this.formData, {
            withCredentials: true
          });
          console.log('Response:', response.data);
          this.fetchUnitOfMeasureCategories();
        } catch (error) {
      ...
      async updateForm() {
        try {
          const response = await axios.put(`http://127.0.0.1:8000/inventorybackend/UoM/update/${this.selectedCategory.id}/`, this.selectedCategory, {
            withCredentials: true
          });
          ...
          console.error('Error updating unit of measure category:', error.response.data);
        }
      },
      async deleteUnitOfMeasureCategory(id) {
        try {
          await axios.delete(`http://127.0.0.1:8000/inventorybackend/UoM/delete/${id}/`, {
            withCredentials: true
          });
          this.fetchUnitOfMeasureCategories();
        } catch (error) {
          console.error('Error deleting unit of measure category:', error.response.data);
        }
      }
    },
    mounted() {
